...
responsabilidade com o servico Portmap (111/udp) habilitado. Este
 ...
 * o servico Portmap seja acessivel apenas `a sua rede local, ou que
...
servidores com o servico Portmap habilitado, por favor repasse essa
 ...
 * portmap: status/pacotes/bytes, onde status e' "open", e
...
* O que e' o servico Portmap (111/udp)?

  O servico Portmap, tambem chamado de Portmapper ou rpcbind, e'
  utilizado para mapear servicos RPC na rede, como NIS e NFS. Esse
  servico geralmente e' utilizado por sistemas Unix, especialmente
  para compartilhamento de arquivos, alem de outros servicos.
  ...
  servico Portmap estar exposto `a Internet.
...
* Como faco para corrigir o problema?
  ...
  Configure o servico Portmap de forma que ele seja acessivel somente
  aos dispositivos de sua rede, que utilizem esse servico, ou que ele
  seja acessivel apenas via TCP. Caso o servico Portmap seja
...
* Como o CERT.br sabe que este e' um servidor Portmap vulneravel?

  O CERT.br esta' recebendo notificacoes com listas de servidores
  Portmap que possivelmente estao sendo abusados e utilizados em
...
* Como posso ter certeza que resolvi o problema?
  ...
  Onde IP_SERVIDOR e' o IP do servidor Portmap a ser testado.
  ...
  rpcinfo (geralmente parte do pacote rpcbind) instalada em seu
  computador.
